 ...
 *
 * REFERENCE NODE SLAVE FIRMWARE
 * ============================================================
 *
 * Hardware Role:
 *   - Reference H-Polarized 1/4-wave antenna radio
 *   - Controlled via Serial1 from Reference Node Master (V-Pol)
 ...
 *      - Execute RF window:
 *        * If phase == PHASE_REF_H_TX: transmit raw 802.11 action frames
 ...
 *      - Send LOCAL_RESULT summary over Serial1 to Master
 *
 * ============================================================
 */
...
static void runRFTransmission(uint64_t startAtUs, uint32_t windowUs, uint32_t intervalUs, uint8_t channel, const uint8_t *dstMac) {
    ...
    while (esp_timer_get_time() < endAt) {
        ...
        uint16_t len = build_raw_80211_action_frame(
            frame, ownMac, dstMac, gRunId, gTestId, (uint8_t)gCurrentPhase, seq, channel, 1 // Role 1 = Ref Slave H
        );
...
static void handleStartAt(const StartAtCommand *cmd) {
    ...
    bool isTx = (gCurrentPhase == PHASE_REF_H_TX);
    if (isTx) {
...
void setup() {
    ...
    Serial.println("Reference Node Slave Firmware (H-Pol Antenna)");
    ...
    Serial.printf("Ref Slave MAC: %02X:%02X:%02X:%02X:%02X:%02X\n",
